...
export default function QuoteForm({
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError("We could not send that. Please call or text us instead.");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <form onSubmit={submit} className="space-y-3" name="quote">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div>
          ...
          <input id="q-name" name="name" required placeholder="Full name" value={data.name} onChange={update} className={inputClass} />
        </div>
        <div>
          <label htmlFor="q-phone" className="sr-only">Phone</label>
          <input id="q-phone" name="phone" type="tel" required placeholder="Phone" value={data.phone} onChange={update} className={inputClass} />
        </div>
        <div>
          <label htmlFor="q-email" className="sr-only">Email</label>
          <input id="q-email" name="email" type="email" required placeholder="Email" value={data.email} onChange={update} className={inputClass} />
        </div>
        <div>
          <label htmlFor="q-zip" className="sr-only">Zip code</label>
          <input id="q-zip" name="zip" inputMode="numeric" pattern="[0-9]{5}" required placeholder="Zip code" value={data.zip} onChange={update} className={inputClass} />
        </div>
      </div>
      <div>
      ...
      {!compact && (
        <div>
          ...
          <textarea id="q-message" name="message" rows={4} placeholder="Bedrooms, bathrooms, pets, parking notes, anything we should know" value={data.message} onChange={update} className={inputClass} />
        </div>
      )}
