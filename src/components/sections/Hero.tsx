...
import { CheckIcon, PhoneIcon, StarIcon } from "@/components/ui/Icons";
...
export default function Hero({
  ...
  return (
    <section className="chart-pattern relative overflow-hidden bg-navy text-white">
      <div
        aria-hidden
        className="pointer-events-none absolute -right-40 -top-40 h-[520px] w-[520px] rounded-full bg-navy-soft/60 blur-3xl"
      />
      <div className="relative mx-auto max-w-7xl px-4 py-14 sm:px-6 md:py-20 lg:px-8">
        ...
        <div className={`grid items-center gap-12 ${withForm ? "lg:grid-cols-12" : ""}`}>
          <div className={withForm ? "lg:col-span-7" : "max-w-3xl"}>
            {eyebrow && <p className="eyebrow mb-4 !text-brick-soft">{eyebrow}</p>}
              ...
              <ul className="mt-7 grid gap-2.5 sm:grid-cols-2">
                {highlights.map((h) => (
                  <li key={h} className="flex items-start gap-2.5 text-sm text-white/90">
                    <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-brick">
            ...
            <div className="mt-8 flex items-center gap-3 text-sm text-white/70">
              ...
              <span>
                {SITE_CONFIG.rating} rating from {SITE_CONFIG.reviewCount} Greater Boston households
              </span>
            </div>
          </div>
            ...
            <div className="lg:col-span-5">
              <div className="rounded-2xl bg-paper p-6 text-ink shadow-2xl sm:p-7">
                ...
                <p className="mb-5 mt-1.5 text-sm text-muted">
                  Same-week availability across most of the city.
                </p>
