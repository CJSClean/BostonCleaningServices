...
import { ClockIcon, FacebookIcon, InstagramIcon, LinkedInIcon, MailIcon, NextdoorIcon, PhoneIcon, PinIcon, XIcon } from "@/components/ui/Icons";
              ...
              <ul className="mt-6 flex items-center gap-3">
                {SOCIAL_PROFILES.map((p) => (
                  <li key={p.name}>
                    ...
                    >
                      {p.name === "instagram" ? <InstagramIcon className="h-4 w-4" /> : p.name === "linkedin" ? <LinkedInIcon className="h-4 w-4" /> : p.name === "x" ? <XIcon className="h-4 w-4" /> : p.name === "nextdoor" ? <NextdoorIcon className="h-4 w-4" /> : <FacebookIcon className="h-4 w-4" />}
                    </a>
                  </li>
                ))}
              </ul>
