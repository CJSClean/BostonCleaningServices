...
function Dropdown({
  ...
  return (
    ...
    >
      <button
        className="flex items-center gap-1 py-2 text-sm font-medium text-ink hover:text-brick"
        aria-expanded={open}
        onClick={() => setOpen((o) => !o)}
      >
        {label}
    ...
    <header className="sticky top-0 z-40">
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-1.5 text-xs sm:px-6 lg:px-8">
          <div className="flex items-center gap-5">
            <span className="hidden items-center gap-1.5 sm:flex">
              <ClockIcon className="h-3.5 w-3.5 text-brick-soft" /> {SITE_CONFIG.hoursShort}
            </span>
            <span className="hidden items-center gap-1.5 md:flex">
              <PinIcon className="h-3.5 w-3.5 text-brick-soft" /> Serving Boston, Cambridge, Somerville, Brookline & Newton
            </span>
          </div>
          <a href={SITE_CONFIG.phoneHref} className="flex items-center gap-1.5 font-semibold hover:text-brick-soft">
      ...
      <div className="border-b border-line bg-paper/95 backdrop-blur">
        <div className="mx-auto flex h-[72px] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          <Link href="/" aria-label="Boston Cleaning Services home" className="shrink-0">
            <Logo className="h-11" />
          </Link>
          ...
          <nav className="hidden items-center gap-7 lg:flex" aria-label="Primary">
            ...
            {NAV_LINKS.main.map((l) => (
              <Link
                key={l.href}
                href={l.href}
                className={`text-sm font-medium hover:text-brick ${pathname === l.href ? "text-brick" : "text-ink"}`}
              >
                {l.name}
              </Link>
            ))}
        ...
        <div className="fixed inset-0 z-50 bg-paper lg:hidden">
          <div className="flex h-[72px] items-center justify-between border-b border-line px-4">
            ...
            <button
              ...
              className="rounded-lg border border-line p-2 text-navy"
            >
              <CloseIcon />
            </button>
          </div>
          <div className="h-[calc(100vh-72px)] overflow-y-auto px-4 pb-10 pt-4">
            ...
            <ul className="mb-8 space-y-1">
              {NAV_LINKS.main.map((l) => (
                <li key={l.href}>
                  <Link href={l.href} onClick={() => setMobileOpen(false)} className="block py-2 text-base font-semibold text-navy">
                    {l.name}
                  </Link>
                </li>
              ))}
            ...
            <a
              ...
              className="mb-3 flex items-center justify-center gap-2 rounded-lg bg-navy px-6 py-3.5 text-sm font-semibold text-white"
            >
